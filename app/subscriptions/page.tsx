    ...
    <DashboardLayout>
      <PageHeader
        ...
        actions={
          <button
            ...
            className="flex items-center gap-2 bg-surface-container text-on-surface-variant font-label font-medium px-4 py-2 rounded-md hover:bg-surface-high transition-colors text-sm border border-outline-variant"
          >
      ...
      <div className="px-8 py-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 bg-surface-container rounded-lg p-1">
            {FY_OPTIONS.map(opt => (
              <button
                ...
                className={cn(
                  'px-4 py-1.5 rounded text-xs font-label font-medium transition-colors',
          ...
          {!loading && (
            <div className="flex items-center gap-2 text-xs text-on-surface-variant">
              ...
              <span>{totalCells} total cells</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-surface-lowest rounded-xl border border-outline-variant/20 shadow-sm">
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-separate border-spacing-0">
                ...
                <thead>
                  <tr>
                    <th className="sticky left-0 z-20 bg-surface-low px-5 py-3 text-left text-xs font-label font-semibold text-on-surface-variant uppercase tracking-wide rounded-tl-xl min-w-48 border-b border-outline-variant/15">
                      Member
                    </th>
                  ...
                  {members.length === 0 ? (
                    ...
                    members.map((member, rowIdx) => (
                      ...
                      >
                        ...
                        <td
                          className={cn(
                            'sticky left-0 z-10 px-5 py-2.5 border-b border-outline-variant/10',
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-7 h-7 rounded-full bg-primary-container flex items-center justify-center shrink-0">
                              <span className="text-on-primary-container text-[9px] font-bold">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {member.is_bod && (
                                  <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-tertiary-fixed text-on-tertiary-fixed-variant shrink-0">
                                    BOD
                                  </span>
                                )}
                              </div>
                        ...
                        {FY_MONTHS.map((month, colIdx) => {
                          ...
                          return (
                            <td
                              key={colIdx}
                              className="px-1 py-2 text-center border-b border-outline-variant/10"
                            >
          ...
          {/* Legend */}
          {!loading && (
            <div className="px-5 py-3 border-t border-outline-variant/15 flex items-center gap-5 flex-wrap">
      ...
      {/* Page-level subscription modal with prefill */}
